...
from Environments.DSSdirect_13bus_loadandswitching.state_action_reward import *
...
class DSS_OutCtrl_Env(gym.Env):
    ...
    def __init__(self):
        print("Initializing 13-bus env for outage management with generators, sectionalizing and tie switches (load control included)")
    ...
    def reset(self):
        ...
        rad=ceil(uniform(0, max_rad)) # select radius of subgraph from a uniform distribution
        ...
        if np.isnan(obs["EdgeFeat(Branchflow)"]).astype(np.int32).sum() > 0 or obs["NodeFeat(BusVoltage)"].max() > 100000: #if the scenario is causing nan as edge variables
            return self.reset()     # reset again
        return obs
