...
class DSS_OutCtrl_Env(gym.Env):
    ...
    def reset(self):
        ...
        for o_e in out_edges:
            ...
            self.DSSCktObj.dss.Text.Command(f'{branch_name}.enabled="False"') #disable the outage line
        ...
        if np.isnan(obs["EdgeFeat(Branchflow)"]).astype(np.int32).sum() > 0: #if the scenario is causing nan as edge variables
